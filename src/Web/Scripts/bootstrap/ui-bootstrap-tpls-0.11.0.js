...
.directive('changeDollarVal', ['$timeout', function ($timeout) {
    return function (scope, element, attrs) {
        if ($('style#dollarStyle').length <= 0) {
            $('head').append('<style type="text/css" id="dollarStyle"></style>');
        }
        $timeout(function () {
            var tempId = $(element).parents('.cprogress-bar').attr('id');
            if (parseInt(tempId.replace('prg', '')) == 0) {
                $('style#dollarStyle').html('');
            }
            $('style#dollarStyle').eq(0).append('#' + tempId + ' .progress-bar:after{height:' + $('#' + tempId).attr('data-val') + '%}');
        });
...
angular.module("template/progressbar/progressbar.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("template/progressbar/progressbar.html",
      "<div class=\"progress\" data-val={{percent}}>\n" +
      "  <div change-dollar-val class=\"progress-bar\" ng-class=\"type && 'progress-bar-' + type\" role=\"progressbar\" aria-valuenow=\"{{value}}\" aria-valuemin=\"0\" aria-valuemax=\"{{max}}\" ng-style=\"{width: (percent < 100 ? percent : 100) + '%'}\" aria-valuetext=\"{{percent | number:0}}%\" style=\"min-width: 0;\" ng-transclude></div>\n" +
      "</div>\n" +
      "");
}]);
